...
@Suite("MeetingMediaSignalFilter", .guesliHermeticSupport)
struct MeetingMediaSignalFilterTests {
    private let now = Date(timeIntervalSince1970: 1_800_000_000)
    private let selfBundleID = "com.guesli.app"
    ...
    @Test("Guesli dictation mic does not satisfy calendar meeting activity")
    func guesliDictationMicDoesNotSatisfyCalendarMeetingActivity() {
        let media = MeetingMediaSignalFilter.apply(
            deviceMicActive: true,
            cameraActive: false,
            audioInputProcesses: [
                audioProcess(bundleID: selfBundleID, appName: "Guesli"),
            ],
    ...
    @Test("external meeting mic still counts when Guesli is also using input")
    func externalMeetingMicStillCountsWhenGuesliIsAlsoUsingInput() {
        let media = MeetingMediaSignalFilter.apply(
            deviceMicActive: true,
            cameraActive: false,
            audioInputProcesses: [
    ...
    @Test("Guesli camera does not satisfy calendar meeting activity")
    func guesliCameraDoesNotSatisfyCalendarMeetingActivity() {
        let media = MeetingMediaSignalFilter.apply(
    ...
    @Test("self helper audio input is treated as Guesli")
    func selfHelperAudioInputIsTreatedAsGuesli() {
        let media = MeetingMediaSignalFilter.apply(
            deviceMicActive: true,
            cameraActive: false,
            audioInputProcesses: [
                audioProcess(bundleID: "\(selfBundleID).helper", appName: "Guesli Helper"),
            ],
            sensorAttributions: sensorAttributions(),
            selfBundleID: selfBundleID
        )
