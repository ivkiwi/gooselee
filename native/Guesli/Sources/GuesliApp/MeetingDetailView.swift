...
struct MeetingDetailView: View {
    ...
    private func summaryFailureBanner(message: String, meeting: MeetingRecord) -> some View {
        HStack(alignment: .top, spacing: GuesliTheme.spacing8) {
            ...
            Spacer(minLength: GuesliTheme.spacing8)
            Button("Try Again") {
        ...
        .background(GuesliTheme.transcribing.opacity(0.10))
    }
    ...
    private func header(_ meeting: MeetingRecord) -> some View {
        ...
        VStack(alignment: .leading, spacing: GuesliTheme.spacing16) {
            ...
            HStack(alignment: .top, spacing: GuesliTheme.spacing24) {
    ...
    private func headerActions(for meeting: MeetingRecord, appliedTemplate: MeetingTemplateSnapshot) -> some View {
        ViewThatFits(in: .horizontal) {
            ...
            VStack(alignment: .trailing, spacing: GuesliTheme.spacing8) {
                HStack(spacing: GuesliTheme.spacing8) {
                    resumeChooserIfAvailable(for: meeting)
                    templateMenu(for: meeting, appliedTemplate: appliedTemplate)
                    exportMenu(for: meeting)
    ...
    private func contentToolbar(for meeting: MeetingRecord) -> some View {
        HStack {
            ...
            Button(action: {
                ...
                .background(
                    RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall)
                        .fill(GuesliTheme.accent.opacity(0.18))
                )
                .overlay(
                    RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall)
                        .strokeBorder(GuesliTheme.accent.opacity(0.35), lineWidth: 1)
                )
            }
            .buttonStyle(.plain)
        }
    ...
    private func statusChip(for meeting: MeetingRecord) -> some View {
        ...
        let color = isPreparing || isPaused ? GuesliTheme.transcribing : meeting.status.displayColor
        HStack(spacing: 6) {
            Circle()
                .fill(color)
    ...
    private var pauseResumeRecordingButton: some View {
        ...
        return Button {
            ...
            .foregroundStyle(isPaused ? GuesliTheme.backgroundBase : GuesliTheme.textPrimary)
            .padding(.horizontal, GuesliTheme.spacing12)
            .padding(.vertical, 7)
            .background(isPaused ? GuesliTheme.accent : GuesliTheme.surfacePrimary)
            .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall))
            .overlay(
                RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall)
                    .strokeBorder(isPaused ? GuesliTheme.accent.opacity(0.35) : GuesliTheme.surfaceBorder, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    ...
    private func folderPill(for meeting: MeetingRecord) -> some View {
        ...
        Button {
            ...
            .foregroundStyle(hasFolder ? GuesliTheme.accent : GuesliTheme.textSecondary)
            .padding(.horizontal, GuesliTheme.spacing8)
            .padding(.vertical, 4)
            .background(hasFolder ? GuesliTheme.accentSubtle : GuesliTheme.backgroundRaised)
            .clipShape(Capsule())
            .overlay(
                Capsule()
                    .strokeBorder(hasFolder ? Color.clear : GuesliTheme.surfaceBorder, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    ...
    private var transcriptCTA: some View {
        ...
        .background(GuesliTheme.accent.opacity(0.08))
        .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall))
    }
...
private struct MeetingTranscriptView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: GuesliTheme.spacing8) {
                if messages.isEmpty {
...
struct TranscriptChatBubble: View {
    ...
    var body: some View {
        HStack(alignment: .bottom, spacing: GuesliTheme.spacing8) {
            if message.isUser {
                Spacer(minLength: 80)
            }
            ...
            .padding(.horizontal, GuesliTheme.spacing12)
            .padding(.vertical, 8)
            .background(message.isUser ? GuesliTheme.accent.opacity(0.18) : GuesliTheme.surfacePrimary)
            .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall))
            .overlay(
                RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall)
                    .strokeBorder(message.isUser ? GuesliTheme.accent.opacity(0.25) : GuesliTheme.surfaceBorder, lineWidth: 1)
            )
