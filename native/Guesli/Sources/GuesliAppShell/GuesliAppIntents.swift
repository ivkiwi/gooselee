...
private enum GuesliShortcutsRuntime {
    @MainActor
    static func waitForController(timeout: TimeInterval = 5) async throws -> GuesliController {
        let deadline = ContinuousClock.now + .seconds(timeout)
        while ContinuousClock.now < deadline {
            if let controller = GuesliController.current { return controller }
            try Task.checkCancellation()
            try await Task.sleep(nanoseconds: 100_000_000)
        }
        guard let controller = GuesliController.current else {
