...
@Suite("Jaro-Winkler Similarity", .guesliHermeticSupport)
struct JaroWinklerTests {
    ...
    func singleChar() {
        ...
        #expect(score2 == 1.0)
        let score3 = CustomWordMatcher.jaroWinklerSimilarity("I", "guesli")
    ...
    func prefixBoost() {
        let withPrefix = CustomWordMatcher.jaroWinklerSimilarity("martha", "marhta")
        let noPrefix = CustomWordMatcher.jaroWinklerSimilarity("xartha", "marhta")
        #expect(withPrefix > noPrefix)
    }
}

@Suite("Custom Word Matcher", .guesliHermeticSupport)
struct CustomWordMatcherApplyTests {

    @Test("exact match replaces word")
    func exactMatch() {
        let words = [CustomWord(word: "museli", replacement: "guesli")]
        let result = CustomWordMatcher.apply(text: "I love museli", customWords: words)
        #expect(result == "I love guesli")
    }
    ...
    func noMatch() {
        let words = [CustomWord(word: "guesli", replacement: "Guesli")]
    ...
    func noReplacement() {
        let words = [CustomWord(word: "kubernetes", replacement: nil)]
        let result = CustomWordMatcher.apply(text: "deploy to kubernete", customWords: words)
        #expect(result == "deploy to kubernetes")
    }
