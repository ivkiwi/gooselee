...
struct LiveTranscriptView: View {
    ...
    private func liveBubble(for group: LiveTranscriptGroup) -> some View {
        ...
        HStack(alignment: .bottom, spacing: 6) {
            ...
            .padding(.vertical, 6)
            .background(isUser ? GuesliTheme.accent.opacity(0.15) : GuesliTheme.surfacePrimary)
            .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall))
            .overlay(
                RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall)
                    .strokeBorder(
                        isUser ? GuesliTheme.accent.opacity(0.2) : GuesliTheme.surfaceBorder,
                        lineWidth: 1
                    )
            )
            if !isUser { Spacer(minLength: 40) }
    ...
    private func partialBubble(speaker: String, text: String, isUser: Bool) -> some View {
        HStack(alignment: .bottom, spacing: 6) {
            ...
            .padding(.horizontal, 10)
            .padding(.vertical, 6)
            .background(isUser ? GuesliTheme.accent.opacity(0.06) : GuesliTheme.surfacePrimary.opacity(0.5))
            .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall))
            .overlay {
                RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall)
                    .strokeBorder(
