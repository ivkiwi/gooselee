...
@Suite("Update failure guidance", .guesliHermeticSupport)
...
@Suite("Update action routing", .guesliHermeticSupport)
struct UpdateActionRoutingTests {
    ...
    func statusBarUpdateActionUsesStandardSparkleFlow() throws {
        ...
        """))
        #expect(statusBarSource.contains("#selector(GuesliController.checkForUpdates)"))
    ...
    func aboutPageUsesMenuBarGuidanceOnly() throws {
        ...
        #expect(!source.contains("let controller: GuesliController"))
...
@Suite("Sidebar hit areas", .guesliHermeticSupport)
