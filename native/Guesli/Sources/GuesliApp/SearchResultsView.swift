...
struct SearchResultsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            header
            Divider().foregroundStyle(GuesliTheme.surfaceBorder)
        ...
        Button {
            ...
            HStack(spacing: 6) {
                Text(tab.rawValue)
                    ...
                    .foregroundStyle(isSelected ? GuesliTheme.textPrimary : GuesliTheme.textTertiary)
                Text("\(count)")
                    .font(.system(size: 11, weight: .medium))
                    .foregroundStyle(isSelected ? GuesliTheme.accent : GuesliTheme.textTertiary)
                    .padding(.horizontal, 6)
                    .padding(.vertical, 2)
                    .background(isSelected ? GuesliTheme.accentSubtle : GuesliTheme.backgroundRaised)
                    .clipShape(Capsule())
            }
            .padding(.horizontal, 12)
            .padding(.vertical, 8)
            .background(isSelected ? GuesliTheme.backgroundHover : Color.clear)
...
private struct SearchDictationRow: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, GuesliTheme.spacing20)
        .padding(.vertical, GuesliTheme.spacing12)
        .background(isHovered ? GuesliTheme.backgroundHover : Color.clear)
        .onHover { hovering in
...
private func snippetText(from text: String, highlighting query: String) -> Text {
    ...
    guard let matchRange = text.range(of: query, options: .caseInsensitive) else {
        ...
        return Text(truncated).foregroundStyle(GuesliTheme.textSecondary)
    }
    ...
    guard let localRange = snippet.range(of: query, options: .caseInsensitive) else {
        return Text(prefix + snippet + suffix).foregroundStyle(GuesliTheme.textSecondary)
    }
    ...
    return Text(prefix + before).foregroundStyle(GuesliTheme.textSecondary)
        + Text(match).bold().foregroundStyle(GuesliTheme.accent)
        + Text(after + suffix).foregroundStyle(GuesliTheme.textSecondary)
}
