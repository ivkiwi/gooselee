...
enum LocalVQEModelStore {
    ...
    static func resolveModelURL(downloadIfMissing: Bool = true) async throws -> URL {
        if let override = ProcessInfo.processInfo.environment["GUESLI_LOCALVQE_MODEL_PATH"],
           !override.isEmpty {
        ...
        GuesliPaths.preconditionSafeForTestWrite(url)
...
enum LocalVQELibraryLocator {
    static func resolve(explicitPath: String? = ProcessInfo.processInfo.environment["GUESLI_LOCALVQE_LIBRARY_PATH"]) throws -> URL {
...
final class LocalVQEAudioProcessor: MeetingAecProcessor {
    ...
    init(modelURL: URL, libraryURL: URL, threads: Int) throws {
        ...
        sampleRate = Int(guesli_localvqe_sample_rate(context))
        frameSize = Int(guesli_localvqe_hop_length(context))
    ...
    deinit {
        if let context {
            guesli_localvqe_destroy(context)
        }
    }

    func reset() {
        guard let context else { return }
        guesli_localvqe_reset(context)
    }
    ...
    func processFrame(mic: [Float], reference: [Float]) throws -> [Float] {
        ...
        let status = mic.withUnsafeBufferPointer { micBuffer in
            reference.withUnsafeBufferPointer { referenceBuffer in
                output.withUnsafeMutableBufferPointer { outputBuffer in
                    guesli_localvqe_process_frame_f32(
                        context,
        ...
        guard status == 0 else {
            throw LocalVQEError.processFailed(String(cString: guesli_localvqe_last_error(context)))
        }
        return output
    }
}
