...
@Suite("MeetingNeuralAec", .guesliHermeticSupport)
struct MeetingNeuralAecTests {
    ...
    func localVQEBridgeRejectsEmptyModelPath() {
        ...
        let context = guesli_localvqe_create("", "", 2, &error, Int32(error.count))
        #expect(context == nil)
    ...
    func localVQEBridgeReportsMissingLibraryPath() throws {
        ...
        var error = [CChar](repeating: 0, count: 512)
        let context = guesli_localvqe_create(
            modelURL.path,
            "/tmp/guesli-missing-localvqe-\(UUID().uuidString).dylib",
            2,
            &error,
            Int32(error.count)
        )
    ...
    private func makeTemporaryAppBundle() throws -> TemporaryAppBundle {
        ...
        <dict>
          ...
          <string>com.guesli.tests.MeetingAec</string>
