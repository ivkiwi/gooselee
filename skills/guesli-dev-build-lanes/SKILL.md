...
2. Inspect `scripts/dev-test.sh`, `scripts/build_native_app.sh`, and `scripts/guesli_spm_cache.sh` before changing build behavior.
...
Named lanes default to local-only entitlements. This uses `scripts/GuesliLocalOnly.entitlements` and clears provisioning/APNs env for the build.
...
GUESLI_CODESIGN_TIMESTAMP=none \
...
If `/Volumes/GuesliBuildCache/guesli-spm` is mounted, prefer it. Otherwise scripts fall back to `~/Library/Caches/guesli-spm`.
