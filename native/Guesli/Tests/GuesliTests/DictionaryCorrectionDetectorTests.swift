...
@Suite("Dictionary correction detector", .guesliHermeticSupport)
struct DictionaryCorrectionDetectorTests {
    ...
    func contractSurfacesSupportedCorrectionShapesOnly() {
        let singleWordSuggestion = DictionaryCorrectionDetector.suggestion(
            ...
            editedText: "for the dictionary test please write guesli in this sentence today"
        )
        #expect(singleWordSuggestion?.observed == "musley")
        #expect(singleWordSuggestion?.replacement == "guesli")
    ...
    func detectsLessSimilarProductNameCorrection() {
        let suggestion = DictionaryCorrectionDetector.suggestion(
            ...
            currentText: "the word guesli is hard to transcribe"
        )
    ...
    @Test("detects guesli corrections from latest failure shape")
    func detectsGuesliCorrectionFromLatestFailureShape() {
        ...
        let edited = "No, typically I think the word guesli is the worst toughest one for it to transcribe because it usually transcribes to the one starting with the w letter N instead of just guesli."
        ...
        #expect(suggestion?.observed == "muzzli")
        #expect(suggestion?.replacement == "guesli")
    }

    @Test("detects guesli corrections from latest musley failure")
    func detectsGuesliCorrectionFromLatestMusleyFailure() {
        ...
        let edited = "Okay, it is able to transcribe the word guesli if I say it very explicitly and enunciate the words, but I think as such if I say guesli fast it's not able to."
    ...
    @Test("detects guesli corrections from latest muwsly failure")
    func detectsGuesliCorrectionFromLatestMuwslyFailure() {
        let original = "See, I typically find the word guesli to be wrongly transcribed almost every time, so I'm just repeating muwsly twice."
        let edited = "See, I typically find the word guesli to be wrongly transcribed almost every time, so I'm just repeating guesli twice."
    ...
    func detectsCorrectionWithAdditionalTyping() {
        ...
        let edited = "So this time if I say guesli has not transcribed guesli properly, would you be able to add it to dictionary immediately? (still did not trigger the prompt unfortunately)"
    ...
    func detectsCorrectionInsideLargeFocusedTextSnapshot() {
        ...
        let edited = "\(prefix) I like guesli today \(suffix)"
    ...
    func detectsFarApartCorrectionsInsideLargeFocusedTextSnapshot() {
        ...
        let edited = "\(prefix) I like guesli today \(middle) I mentioned guesli again \(suffix)"
        ...
        #expect(suggestions.map(\.replacement) == ["guesli", "guesli"])
    }
    ...
    func detectsFarApartCorrectionsAcrossRepeatedSeparatorText() {
        ...
        let edited = "I like guesli today \(middle) I mentioned guesli again"
    ...
    func detectsCorrectionNextToInsertedWord() {
        let suggestions = DictionaryCorrectionDetector.suggestions(
            ...
            editedText: "I like fresh guesli today",
            maxSuggestions: 3
        )

        #expect(suggestions.map(\.observed) == ["museli"])
        #expect(suggestions.map(\.replacement) == ["guesli"])
    }
    ...
    func requiresSharedDictationContext() {
        #expect(DictionaryCorrectionDetector.hasSufficientSharedContext(
            ...
            editedText: "So this time if I say guesli has not transcribed guesli properly and then I kept typing"
        ))
        ...
        #expect(!DictionaryCorrectionDetector.hasSufficientSharedContext(
            originalText: "hello guesli today",
    ...
    func detectsFinalEditedTranscriptSnapshot() {
        let suggestion = DictionaryCorrectionDetector.suggestion(
            ...
            editedText: "Hey Guesli, can you see if you're transcribing the word correctly?"
        )
    ...
    func ignoresOutsideEdits() {
        let suggestion = DictionaryCorrectionDetector.suggestion(
            originalText: "hello guesli",
            baselineText: "before hello guesli",
            currentText: "before hello guesli!"
        )
...
@Suite("Dictionary correction snapshot stabilizer", .guesliHermeticSupport)
struct DictionaryCorrectionSnapshotStabilizerTests {
    ...
    func evaluatesLaterStableSnapshotAfterEarlierCandidate() {
        ...
        let later = "I changed museli to guesli."
...
@Suite("Dictionary suggestion config", .guesliHermeticSupport)
struct DictionarySuggestionConfigTests {
    ...
    func suggestionDecodeToleratesBadIDs() throws {
        ...
        #expect(config.dictionarySuggestions[0].replacement == "guesli")
    ...
    func stableSuggestionKey() {
        #expect(
            DictionarySuggestion.key(observed: " Museli ", replacement: "Guesli")
                == DictionarySuggestion.key(observed: "museli", replacement: " guesli ")
        )
    }
