...
final class MediaPlaybackController: MediaPlaybackManaging {
    ...
    init(
        ...
        queue: DispatchQueue = DispatchQueue(label: "com.guesli.media-playback")
    ) {
        self.client = client
        self.queue = queue
    }
    ...
    private func handleRestorePlaybackState(_ playbackState: MediaPlaybackState, token: Int) {
        ...
        // leaving media stranded after Guesli paused it.
...
private final class NowPlayingMediaRemoteClient {
    ...
    private let timeoutQueue = DispatchQueue(label: "com.guesli.media-playback.now-playing-timeout")
