...
@Suite("Meeting resume and follow-up", .serialized, .guesliHermeticSupport)
struct MeetingResumeAndFollowUpTests {
    private func makeStore() throws -> DictationStore {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-resume-follow-up-\(UUID().uuidString).db")
    ...
    func legacyMigration() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-follow-up-legacy-\(UUID().uuidString).db")
        var db: OpaquePointer?
