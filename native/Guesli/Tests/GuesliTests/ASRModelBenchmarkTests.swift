...
@Suite("ASR model efficiency benchmarks", .serialized, .guesliHermeticSupport)
struct ASRModelEfficiencyBenchmarks {
    ...
    func compareProductionASRModelsOnRealRecordings() async throws {
        ...
        guard environment["GUESLI_ASR_BENCH"] == "1" else {
            return
        }

        let config = try BenchmarkConfig(environment: environment)
        let recordings = try config.recordings()
        #expect(!recordings.isEmpty, "Set GUESLI_ASR_BENCH_RECORDINGS or keep readable meetings in the configured DB.")
...
private struct BenchmarkConfig {
    ...
    init(environment: [String: String]) throws {
        databaseURL = URL(
            fileURLWithPath: environment["GUESLI_ASR_BENCH_DB"]?.expandedPath
                ?? "~/Library/Application Support/Guesli/guesli.db".expandedPath
        )
        outputURL = URL(
            fileURLWithPath: environment["GUESLI_ASR_BENCH_OUTPUT"]?.expandedPath
                ?? "/tmp/guesli-asr-bench.jsonl"
        )
        reportURL = URL(
            fileURLWithPath: environment["GUESLI_ASR_BENCH_REPORT"]?.expandedPath
                ?? outputURL.deletingPathExtension().appendingPathExtension("md").path
        )
        modelIDs = Self.csv(environment["GUESLI_ASR_BENCH_MODELS"])
        meetingIDs = Self.csv(environment["GUESLI_ASR_BENCH_MEETING_IDS"]).compactMap(Int64.init)
        recordingPaths = Self.csv(environment["GUESLI_ASR_BENCH_RECORDINGS"]).map(\.expandedPath)
        limit = Int(environment["GUESLI_ASR_BENCH_LIMIT"] ?? "") ?? 2
        startSeconds = max(Double(environment["GUESLI_ASR_BENCH_START_SECONDS"] ?? "") ?? 0, 0)
        if let raw = environment["GUESLI_ASR_BENCH_MAX_SECONDS"], let value = Double(raw), value > 0 {
            maxSeconds = value
        } else {
            maxSeconds = nil
        }
        allowDownloads = environment["GUESLI_ASR_BENCH_ALLOW_DOWNLOADS"] == "1"
        includeText = environment["GUESLI_ASR_BENCH_INCLUDE_TEXT"] == "1"
    }
...
private struct PreparedBenchmarkAudio {
    ...
    static func make(recording: BenchmarkRecording, startSeconds: Double, maxSeconds: Double?) async throws -> PreparedBenchmarkAudio {
        ...
        let truncatedURL = try WavWriter.writeTemporaryWAV(samples: truncatedSamples, directoryName: "guesli-asr-bench")
...
private func diskURL(for option: BackendOption) -> URL? {
    ...
    case .nemotron35Multilingual:
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".cache/guesli/models/nemotron35-multilingual-2240ms", isDirectory: true)
    default:
        return nil
    }
}
