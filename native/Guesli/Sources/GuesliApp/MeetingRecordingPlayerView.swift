...
enum RecordingWaveformCacheFiles {
    ...
    static func removeAllCachedWaveforms(
        ...
        GuesliPaths.preconditionSafeForTestWrite(directory)
    ...
    static func sweepStaleCachedWaveforms(
        ...
        for entry in entries where entry.pathExtension.lowercased() == "mwf" && isOlder(entry, than: cutoff) {
            do {
                GuesliPaths.preconditionSafeForTestWrite(entry)
        ...
        if removed > 0 {
            logger?(
                "[guesli-native] cleaned up \(removed) stale waveform cache item\(removed == 1 ? "" : "s")"
            )
        }
        return removed
    }
...
private actor RecordingWaveformCache {
    ...
    private func persist(_ waveform: RecordingWaveformData, to cacheURL: URL) {
        do {
            GuesliPaths.preconditionSafeForTestWrite(cacheURL)
...
struct MeetingRecordingPlayerView: View {
    ...
    var body: some View {
        ...
        .background(GuesliTheme.backgroundRaised.opacity(0.74))
        ...
        GeometryReader { proxy in
            Canvas { context, size in
                ...
                for index in 0..<barCount {
                    ...
                    let color = x <= playedX ? GuesliTheme.accent : GuesliTheme.textTertiary.opacity(0.24)
                    context.fill(
