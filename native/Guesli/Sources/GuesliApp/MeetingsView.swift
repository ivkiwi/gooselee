...
struct MeetingsView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: GuesliTheme.spacing24) {
                ...
                if filteredMeetings.isEmpty {
                    ...
                    LazyVStack(spacing: GuesliTheme.spacing12) {
        ...
        VStack(alignment: .leading, spacing: GuesliTheme.spacing16) {
            ...
            ForEach(groups) { group in
                HStack(alignment: .top, spacing: 20) {
                    ...
                    VStack(alignment: .center, spacing: 2) {
                        Text(group.dayNumber)
                            ...
                            .foregroundStyle(group.isToday ? GuesliTheme.accent : GuesliTheme.textPrimary)
                        Text(group.dayLabel)
                            .font(.system(size: 11, weight: .medium))
                            .foregroundStyle(group.isToday ? GuesliTheme.accent : GuesliTheme.textSecondary)
                        ...
                        ForEach(group.events) { event in
                            HStack(spacing: 8) {
                                RoundedRectangle(cornerRadius: 1.5)
                                    .fill(group.isToday ? GuesliTheme.accent : GuesliTheme.textSecondary.opacity(0.4))
                                    ...
                                    Text("Add to folder")
                                        ...
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 4)
                                                .strokeBorder(GuesliTheme.surfaceBorder, lineWidth: 0.5)
                                        )
                                }
                ...
                if group.id != lastGroupId {
                    Divider()
                        .foregroundStyle(GuesliTheme.surfaceBorder)
                }
            }
        }
        .padding(20)
    ...
    private func hideEventButton(_ event: UnifiedCalendarEvent) -> some View {
        Button {
            ...
            Image(systemName: "xmark")
                ...
                .foregroundStyle(GuesliTheme.textSecondary.opacity(0.6))
    ...
    private var browserHeader: some View {
        VStack(alignment: .leading, spacing: GuesliTheme.spacing8) {
            ViewThatFits(in: .horizontal) {
                HStack(alignment: .top, spacing: GuesliTheme.spacing16) {
                    browserHeaderTitle
    ...
    private var browserHeaderActions: some View {
        HStack(spacing: GuesliTheme.spacing8) {
            Button {
                ...
                .foregroundStyle(GuesliTheme.backgroundBase)
                .padding(.horizontal, GuesliTheme.spacing12)
                .padding(.vertical, 8)
                .background(appState.isMeetingRecording || appState.isMeetingStarting ? GuesliTheme.surfacePrimary : GuesliTheme.accent)
        ...
        HStack(spacing: GuesliTheme.spacing12) {
            ...
            if phase == .recording || phase == .paused {
                Button {
                    ...
                    .foregroundStyle(appState.isMeetingRecordingPaused ? GuesliTheme.backgroundBase : GuesliTheme.textPrimary)
                    .padding(.horizontal, GuesliTheme.spacing12)
                    .padding(.vertical, 8)
                    .background(appState.isMeetingRecordingPaused ? GuesliTheme.accent : GuesliTheme.surfacePrimary)
                    .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall))
                    .overlay(
                        RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall)
                            .strokeBorder(appState.isMeetingRecordingPaused ? GuesliTheme.accent.opacity(0.35) : GuesliTheme.surfaceBorder, lineWidth: 1)
                    )
                }
                .buttonStyle(.plain)
                ...
                Button {
                    ...
                    .padding(.horizontal, GuesliTheme.spacing12)
                    .padding(.vertical, 8)
                    .background(GuesliTheme.recording)
                    .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerSmall))
                }
                .buttonStyle(.plain)
                .disabled(!appState.isMeetingRecording)
    ...
    private func activeMeetingStatusColor(for meeting: MeetingRecord) -> Color {
        switch activeMeetingBannerPhase(for: meeting) {
        case .preparing, .paused: return GuesliTheme.transcribing
        case .recording: return GuesliTheme.recording
        case .finalizing: return GuesliTheme.accent
        }
    }
    ...
    private var sortButton: some View {
        Menu {
            ...
            .foregroundStyle(selectedSort != .newestFirst ? GuesliTheme.accent : GuesliTheme.textSecondary)
            .padding(.horizontal, 8)
            .padding(.vertical, 3)
            .background(selectedSort != .newestFirst ? GuesliTheme.accent.opacity(0.12) : GuesliTheme.surfacePrimary.opacity(0.5))
            .clipShape(Capsule())
        }
        .menuStyle(.borderlessButton)
        .fixedSize()
    ...
    private var dateFilterButton: some View {
        Menu {
            ...
            .foregroundStyle(selectedFilter != .all ? GuesliTheme.accent : GuesliTheme.textTertiary)
            .padding(.horizontal, selectedFilter != .all ? 8 : 0)
            .padding(.vertical, 3)
            .background(selectedFilter != .all ? GuesliTheme.accent.opacity(0.12) : Color.clear)
            .clipShape(Capsule())
        }
        .menuStyle(.borderlessButton)
        .fixedSize()
    ...
    private var emptyState: some View {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(GuesliTheme.backgroundRaised)
        .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerXL))
        .overlay(
            RoundedRectangle(cornerRadius: GuesliTheme.cornerXL)
                .strokeBorder(GuesliTheme.surfaceBorder, lineWidth: 1)
        )
    }
}
