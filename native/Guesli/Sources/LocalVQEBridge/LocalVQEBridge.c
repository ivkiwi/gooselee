...
static void context_error(GuesliLocalVQEContext *context, const char *message) {
    if (context != NULL) {
...
GuesliLocalVQEContext *guesli_localvqe_create(
    ...
    GuesliLocalVQEContext *context = (GuesliLocalVQEContext *)calloc(1, sizeof(GuesliLocalVQEContext));
    if (context == NULL) {
        free_context(localvqe);
        dlclose(library);
        set_error(error_buffer, error_buffer_length, "Could not allocate Guesli LocalVQE context");
        return NULL;
    }
...
int guesli_localvqe_process_frame_f32(
    ...
    int hop = guesli_localvqe_hop_length(context);
    if (hop_samples != hop) {
