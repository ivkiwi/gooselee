...
final class MeetingSessionDiagnostics {
    ...
    init(title: String, startedAt: Date) {
        ...
        do {
            GuesliPaths.preconditionSafeForTestWrite(runDirectory)
            ...
            GuesliPaths.preconditionSafeForTestWrite(cleanedURL)
    ...
    private static func pruneOldRuns(in diagnosticsRoot: URL, preserving preservedRun: URL) {
        ...
        for run in runs {
            ...
            if shouldKeep {
                ...
                GuesliPaths.preconditionSafeForTestWrite(run.url)
