...
struct GoogleCalendarCredentials {
    ...
    /// Load credentials from app bundle (production) or ~/.config/guesli/ (dev).
    ...
    static func load() -> GoogleCalendarCredentials? {
        ...
        let devPath = NSString("~/.config/guesli/google-oauth.json").expandingTildeInPath
        let devURL = URL(fileURLWithPath: devPath)
        if let creds = parse(url: devURL) {
            fputs("[google-cal] credentials loaded from ~/.config/guesli/\n", stderr)
            return creds
        }
