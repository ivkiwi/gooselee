...
import Testing
import GuesliCore
@testable import GuesliCLI
@testable import GuesliApp

@Suite("GuesliCLI", .serialized, .guesliHermeticSupport)
struct GuesliCLITests {
    @Test("spec exposes the agent-facing command set")
    func specPayloadIncludesCommands() {
        let payload = GuesliCLI.specPayload()
    ...
    func cliContextUsesExplicitDatabasePath() {
        let context = CLIContext(
            dbPath: "/tmp/custom-guesli.db",
            supportDir: "/tmp/ignored-support"
        )

        #expect(context.databaseURL.path == "/tmp/custom-guesli.db")
    ...
    func cliContextUsesExplicitSupportDirectory() {
        let context = CLIContext(
            dbPath: nil,
            supportDir: "/tmp/guesli-support"
        )

        #expect(context.supportDirectory.path == "/tmp/guesli-support")
        #expect(context.databaseURL.path == "/tmp/guesli-support/guesli.db")
    }
    ...
    @Test("transcribe defaults to ONNX GigaAM")
    func transcribeDefaultsToONNXGigaAM() throws {
        let command = try TranscribeCommand.parse(["recording.wav"])
        #expect(command.model == .gigaAMONNX)
    }

    @Test("partial output is restricted to Parakeet EOU")
    func partialOutputRequiresStreamingModel() throws {
        #expect(throws: Error.self) {
            _ = try TranscribeCommand.parse(["recording.wav", "--emit-partials", "/tmp/partials.jsonl"])
        }
    ...
    func eouWAVReaderStreamsBoundedChunks() async throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-cli-stream-\(UUID().uuidString).wav")
    ...
    func partialJSONLRerunTruncatesStaleTail() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-cli-partials-\(UUID().uuidString).jsonl")
        defer { try? FileManager.default.removeItem(at: url) }
    ...
    func loadCustomWordsAcceptsPlainArray() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-cli-dictionary-\(UUID().uuidString).json")
        try Data("""
        [{"word": "museli", "replacement": "guesli", "matching_threshold": 0.85}]
        """.utf8).write(to: url)

        let words = try GuesliAudioTranscriptionPipeline.loadCustomWords(from: url)
        #expect(words.count == 1)
        #expect(words[0].word == "museli")
        #expect(words[0].targetWord == "guesli")
    }
    ...
    func loadCustomWordsRejectsMissingFile() {
        ...
        #expect(throws: Error.self) {
            _ = try GuesliAudioTranscriptionPipeline.loadCustomWords(from: url)
        }
    }
    ...
    func loadCustomWordsRejectsUnreadablePath() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-cli-dictionary-directory-\(UUID().uuidString)", isDirectory: true)
    ...
    func pipelineValidatesDictionaryBeforeTranscription() async throws {
        ...
        let pipeline = GuesliAudioTranscriptionPipeline(
        ...
        do {
            _ = try await pipeline.run(
                request: GuesliAudioTranscriptionRequest(
    ...
    func pipelineAppliesDictionary() async throws {
        ...
        try Data("""
        [{"word": "museli", "replacement": "guesli"}]
        ...
        #expect(result.transcript == "I love guesli")
    }
    ...
    func transcribeTextOutputIsTranscriptOnly() throws {
        let result = GuesliAudioTranscriptionResult(
            title: "Demo",
            transcript: "hello from guesli",
            summary: nil,
            durationSeconds: 2,
            wordCount: 3,
            model: .parakeetV3,
        ...
        #expect(result.textOutput == "hello from guesli\n")
    }
    ...
    func transcribeJSONPayloadUsesEnvelope() throws {
        let payload = TranscribeJSONPayload(
            GuesliAudioTranscriptionResult(
                title: "Demo",
                transcript: "hello from guesli",
                summary: "## Summary\n\n- Done",
                durationSeconds: 4,
        ...
        let envelope = SuccessEnvelope(
            command: "guesli-cli transcribe",
            data: payload,
            meta: MetaBody(schemaVersion: 1, generatedAt: "2026-07-08T00:00:00Z", dbPath: "/tmp/guesli.db", warnings: ["summary warning"])
        )
        let data = try encodedJSON(envelope)
        let json = try #require(JSONSerialization.jsonObject(with: data) as? [String: Any])

        #expect(json["ok"] as? Bool == true)
        #expect(json["command"] as? String == "guesli-cli transcribe")
        let payloadData = try #require(json["data"] as? [String: Any])
        #expect(payloadData["transcript"] as? String == "hello from guesli")
    ...
    func transcribeOutputWritesFile() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-cli-output-\(UUID().uuidString)", isDirectory: true)
...
private struct TranscribeFixture {
    ...
    init() throws {
        directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-cli-test-\(UUID().uuidString)", isDirectory: true)
        ...
        context = CLIContext(
            dbPath: directory.appendingPathComponent("guesli.db").path,
...
private struct FailingSummarizer: MeetingSummarizing {
    func summarize(transcript: String, title: String, supportDirectory: URL) async throws -> String {
        throw NSError(domain: "GuesliCLITests", code: 1, userInfo: [NSLocalizedDescriptionKey: "summary backend unavailable"])
    }
}
