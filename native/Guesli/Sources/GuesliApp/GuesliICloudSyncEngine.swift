...
final class UserDefaultsICloudChangeTokenStore: ICloudChangeTokenStore {
    ...
    init(
        ...
        key: String = "guesli.icloud.textRecords.GuesliSyncZone.serverChangeToken.v1"
    ) {
...
struct GuesliBridgeDeviceSnapshot: Equatable {
    let deviceID: String
    let platform: String
...
enum GuesliBridgeDeviceIdentity {
    private static let localDeviceIDKey = "guesli.sync.bridge.localDeviceID.v1"
    private static let localDeviceNameKey = "guesli.sync.bridge.localDeviceName.v1"
    private static let remoteDeviceIDKey = "guesli.sync.bridge.remoteDeviceID.v1"
    private static let remoteDeviceNameKey = "guesli.sync.bridge.remoteDeviceName.v1"
    private static let remoteDevicePlatformKey = "guesli.sync.bridge.remoteDevicePlatform.v1"
    private static let remoteDeviceLastSeenAtKey = "guesli.sync.bridge.remoteDeviceLastSeenAt.v1"
    private static let lastRefreshKey = "guesli.sync.bridge.lastRefreshed.v1"
    private static let lastRefreshFailureKey = "guesli.sync.bridge.lastRefreshFailure.v1"
    ...
    static func local(defaults: UserDefaults = .standard) -> GuesliBridgeDeviceSnapshot {
        let deviceID: String
        ...
        return GuesliBridgeDeviceSnapshot(
            deviceID: deviceID,
            platform: "macOS",
    ...
    static func snapshot(from record: CKRecord) -> GuesliBridgeDeviceSnapshot? {
...
final class GuesliICloudSyncEngine {
    private enum Schema {
        static let containerIdentifier = "iCloud.com.gueslihq.guesli"
        static let syncZoneName = "GuesliSyncZone"
        static let textRecordType = "GuesliTextRecord"
        static let bridgeDeviceRecordType = "GuesliBridgeDevice"
        static let textSubscriptionID = "guesli-text-records-sync-zone-v1"
        static let migratedDefaultZoneKey = "guesli.icloud.textRecords.defaultToSyncZoneMigrated.v1"
    ...
    private func refreshBridgeDeviceLink(forceRefresh: Bool = false) async {
        guard GuesliBridgeDeviceIdentity.shouldRefresh(
            defaults: defaults,
        ...
        do {
            ...
            GuesliBridgeDeviceIdentity.updateRemoteDevices(from: records, defaults: defaults)
            GuesliBridgeDeviceIdentity.markRefreshed(defaults: defaults)
        } catch {
            fputs("Failed to refresh iCloud bridge device identity: \(error)\n", stderr)
            GuesliBridgeDeviceIdentity.markRefreshFailed(defaults: defaults)
        }
    }

    private func upsertLocalBridgeDeviceRecord() async throws {
        let snapshot = GuesliBridgeDeviceIdentity.local(defaults: defaults)
    ...
    private func fetchZoneChangesPage(
        ...
        try await withCheckedThrowingContinuation { continuation in
            ...
            operation.recordWithIDWasDeletedBlock = { _, _ in
                // Sync contract: Guesli clients must delete text records by writing
