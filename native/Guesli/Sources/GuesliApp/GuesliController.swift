...
enum DictationBackendReadiness: Equatable {
    ...
    var allowsDictation: Bool { self == .ready }

    func blockingMessage(backendLabel: String) -> String? {
        switch self {
        case .preparing: return "Warming up \(backendLabel)..."
        case .ready: return nil
        case .failed: return "\(backendLabel) unavailable"
        }
    }
}
...
enum GuesliBridgeDeviceRefreshPolicy {
...
private final class DictationLatencyLogWriter: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.guesli.dictation-latency-log")
    private let url: URL
...
@MainActor
public final class GuesliController: NSObject {
    public static weak var current: GuesliController?
    ...
    private static let dictionarySuggestionLogger = Logger(subsystem: "com.guesli.native", category: "DictionarySuggestion")
    ...
    init(
        ...
        let databaseURL = (try? GuesliPaths.migrateHistoricalDatabaseNameIfNeeded(
            supportDirectory: configStore.supportDirectory()
        )) ?? GuesliPaths.databaseURL(supportDirectory: configStore.supportDirectory())
        ...
        if loadedConfig.recordingColorHex != "1e1e2e" {
            GuesliTheme.accentOverrideHex = loadedConfig.recordingColorHex
        }
    ...
    func start() {
        hasStarted = true
        GuesliController.current = self
        do {
            try dictationStore.migrateIfNeeded()
        } catch {
            fputs("[guesli-native] startup error: \(error)\n", stderr)
        }
        ...
        dataDidChangeObserver = DistributedNotificationCenter.default().addObserver(
            forName: GuesliNotifications.dataDidChange,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            Task { @MainActor [weak self] in
    ...
    private func refreshICloudBridgeDeviceState() {
        appState.iCloudBridgeRemoteDeviceName = GuesliBridgeDeviceIdentity.remoteDeviceDisplayName
        appState.iCloudBridgeRemoteDevicePlatform = GuesliBridgeDeviceIdentity.remoteDevicePlatform
    }
    ...
    func recoverStaleLiveMeetings() {
        ...
        do {
            ...
            fputs("[guesli-native] failed to load stale live meetings: \(error)\n", stderr)
            return
        }
        ...
        for meeting in meetings {
            do {
                ...
                fputs("[guesli-native] failed to recover stale meeting \(meeting.id): \(error)\n", stderr)
            }
        }
    ...
    nonisolated static func availableMeetingTranscriptionBackend(
        ...
        downloadedOptions: [BackendOption] = BackendOption.downloadedMeetingTranscription
    ) -> BackendOption? {
        let eligibleOptions = downloadedOptions.filter(\.supportsMeetingTranscription)
        ...
        return BackendOption.resolveDownloaded(
            ...
            fallback: fallback,
            downloadedOptions: eligibleOptions
        )
    }

    nonisolated private static func fallbackMeetingTranscriptionBackend(
        configured: BackendOption?,
        dictationBackend: BackendOption
    ) -> BackendOption {
        if let configured, configured.supportsMeetingTranscription { return configured }
        if dictationBackend.supportsMeetingTranscription { return dictationBackend }
        return BackendOption.all.first(where: \.supportsMeetingTranscription) ?? .gigaAMV3Russian
    }
    ...
    private func normalizeMeetingTranscriptionSelectionForAvailability(
        ...
        if config.meetingTranscriptionBackend != resolved.backend ||
            ...
            fputs("[guesli-native] meeting transcription model unavailable; switched to \(resolved.label)\n", stderr)
        }
    ...
    func updateConfig(_ mutate: (inout AppConfig) -> Void) {
        ...
        configStore.save(config)
        GuesliTheme.accentOverrideHex = config.recordingColorHex == "1e1e2e" ? nil : config.recordingColorHex
    ...
    func enableICloudSync() {
        ...
        iCloudSubscriptionTask = Task { [weak self] in
            do {
                try await GuesliICloudSyncEngine().ensureTextRecordSubscription()
                await MainActor.run {
                ...
                await MainActor.run {
                    ...
                    if GuesliICloudSyncEngine.isICloudAccountAvailabilityError(error) {
    ...
    func handleICloudRemoteNotification(userInfo: [AnyHashable: Any]) {
        guard appleCloudCapabilities.canUseCloudKit,
              ...
              GuesliICloudSyncEngine.isTextRecordSubscriptionNotification(userInfo) else {
            return
        }
        scheduleICloudSync(delay: 0.2, userInitiated: false)
    }
    ...
    private func ensureICloudSubscription() {
        ...
        iCloudSubscriptionTask = Task { [weak self] in
            do {
                ...
                fputs("[guesli-native] failed to ensure iCloud sync subscription: \(error)\n", stderr)
                await MainActor.run {
                    self?.iCloudSubscriptionTask = nil
                }
            }
    ...
    private func startICloudSync(userInitiated: Bool) {
        ...
        let hasKnownCompanionDeviceAtStart = GuesliBridgeDeviceIdentity.hasCompanionRemoteDevice()
        iCloudSyncTask = Task { [weak self] in
            do {
                let forceBridgeDeviceRefresh = GuesliBridgeDeviceRefreshPolicy.shouldForceRefresh(
                ...
                let result = try await GuesliICloudSyncEngine().sync(
                    store: store,
                ...
                do {
                    ...
                    fputs("[guesli-native] failed to purge old iCloud tombstones: \(error)\n", stderr)
                }
                await MainActor.run {
                    guard let self, self.iCloudSyncGeneration == generation else { return }
                    ...
                    let remoteDeviceName = GuesliBridgeDeviceIdentity.remoteDeviceDisplayName ?? "iPhone"
    ...
    private func prepareDictationBackend(_ backend: BackendOption) async -> Bool {
        do {
            ...
            fputs("[guesli-native] dictation backend preparation failed for \(backend.backend)/\(backend.model): \(error)\n", stderr)
            guard selectedBackend == backend else { return false }
    ...
    func selectMeetingTranscriptionBackend(_ option: BackendOption, requireDownloaded: Bool = true) {
        guard option.supportsMeetingTranscription else {
            presentErrorAlert(
                ...
                message: "\(option.label) is a live dictation model and cannot transcribe recordings."
            )
            normalizeMeetingTranscriptionSelectionForAvailability()
            return
        }
    ...
    func signInWithChatGPT() async -> String? {
        do {
            ...
            DiagnosticsLog.write("[guesli-native] ChatGPT sign-in failed: \(error.localizedDescription)")
    ...
    func signInWithGoogleCalendar() async -> String? {
        do {
            ...
            DiagnosticsLog.write("[guesli-native] Google Calendar sign-in failed: \(error.localizedDescription)")
            return error.localizedDescription
        }
    }
    ...
    func refreshGoogleCalendarList() async {
        ...
        do {
            ...
            invalidateGoogleCalendarAuth()
            DiagnosticsLog.write("[guesli-native] Google Calendar token invalid while loading calendar list, signed out")
        } catch GoogleCalendarAuthError.refreshFailed(let message) {
            DiagnosticsLog.write("[guesli-native] Google Calendar token refresh failed while loading calendar list: \(message)")
            appState.googleCalendarListLoadState = .failed("Token refresh failed: \(message)")
        } catch {
            DiagnosticsLog.write("[guesli-native] Google calendarList fetch failed: \(error.localizedDescription)")
    ...
    func refreshUpcomingCalendarEvents(reason: String = "manual") async -> Bool {
        ...
        if googleCalAuth.isAuthenticated {
            do {
                ...
                invalidateGoogleCalendarAuth()
                DiagnosticsLog.write("[guesli-native] Google Calendar token invalid, signed out")
            } catch GoogleCalendarAuthError.refreshFailed(let message) {
                DiagnosticsLog.write("[guesli-native] Google Calendar token refresh failed: \(message)")
            } catch GoogleCalendarClientError.staleRequest {
                return false
            } catch {
                DiagnosticsLog.write("[guesli-native] Google Calendar fetch failed: \(error.localizedDescription)")
            }
        }
    ...
    func continueModelPreparationAfterOnboarding(
        ...
        onboardingModelPreparationTask = Task { [weak self] in
            ...
            do {
                ...
                fputs("[guesli-native] post-onboarding model preparation failed: \(error)\n", stderr)
            }
        }
    }
    ...
    func relaunchApp() {
        ...
        DispatchQueue.main.async {
            ...
            do {
                ...
                fputs("[guesli-native] relaunch failed: \(error)\n", stderr)
            }
    ...
    func downloadModelForOnboarding(
        ...
        guard backend.isDownloaded else {
            throw NSError(
                domain: "GuesliOnboardingModelDownload",
                code: 1,
    ...
    private func resummarize(
        ...
        meetingSummaryTasks[meeting.id] = Task { [weak self] in
            ...
            do {
                ...
                self.meetingSummaryRequestGate.finish(requestID, meetingID: meeting.id)
                self.meetingSummaryTasks.removeValue(forKey: meeting.id)
                DiagnosticsLog.write("[guesli-native] failed to generate or persist meeting summary: \(error.localizedDescription)")
                if error is MeetingSummaryError {
                    do {
                        ...
                        DiagnosticsLog.write("[guesli-native] failed to persist meeting summary error: \(error.localizedDescription)")
                    }
                }
                await MainActor.run {
                    if error is MeetingSummaryError {
        ...
        Task { @MainActor [weak self] in
            ...
            do {
                ...
                do {
                    ...
                    DiagnosticsLog.write("[guesli-native] re-transcription summary generation failed: \(error.localizedDescription)")
                ...
                DiagnosticsLog.write("[guesli-native] failed to re-transcribe meeting \(meeting.id): \(error.localizedDescription)")
    ...
    private func retranscribePreparedMeetingAudio(
        ...
        guard let vadManager = await transcriptionCoordinator.getVadManager() else {
            DiagnosticsLog.write("[guesli-native] retranscribe VAD unavailable; using full-file transcription meeting_id=\(meeting.id)")
        ...
        DiagnosticsLog.write("[guesli-native] retranscribe VAD segments meeting_id=\(meeting.id) count=\(speechSegments.count)")
    ...
    private func retranscribePostModeSourceTracks(
        ...
        if !trimmed.isEmpty {
            if let reason = MeetingRetranscriptionPipeline.suspiciousTranscriptReason(
                ...
                DiagnosticsLog.write("[guesli-native] source-track retranscribe quality_guard_failed meeting_id=\(meeting.id) \(reason)")
    ...
    private func transcribeMeetingSourceTrack(
        ...
        guard let url else {
            DiagnosticsLog.write("[guesli-native] source-track retranscribe track_missing meeting_id=\(meetingID) track=\(trackRole)")
        ...
        DiagnosticsLog.write(
            "[guesli-native] source-track retranscribe track_start meeting_id=\(meetingID) track=\(trackRole) backend=\(backend.backend) model=\(backend.model) file=\(url.lastPathComponent) sample_rate=\(wavData.sampleRate) duration=\(String(format: "%.1f", audioStats.duration)) rms=\(String(format: "%.6f", audioStats.rms)) peak=\(String(format: "%.6f", audioStats.peak)) nonzero=\(String(format: "%.3f", audioStats.nonZeroRatio))"
        )
        guard !wavData.samples.isEmpty else {
            DiagnosticsLog.write("[guesli-native] source-track retranscribe track_empty meeting_id=\(meetingID) track=\(trackRole)")
            return MeetingSourceTrackTranscription(segments: [], samples: [], duration: 0, vadSegmentCount: 0, coveredDuration: 0)
        }
        guard let vadManager = await transcriptionCoordinator.getVadManager() else {
            DiagnosticsLog.write("[guesli-native] source-track retranscribe VAD unavailable; transcribing full \(trackRole) track meeting_id=\(meetingID)")
            let transcription = try await transcriptionCoordinator.transcribeMeeting(
                at: url,
                samples: wavData.samples,
                backend: backend,
            ...
            DiagnosticsLog.write(
                "[guesli-native] source-track retranscribe full_track_done meeting_id=\(meetingID) track=\(trackRole) segments=\(segments.count) chars=\(MeetingRetranscriptionPipeline.characterCount(segments)) text_chars=\(transcription.text.count)"
            )
        ...
        DiagnosticsLog.write(
            "[guesli-native] source-track retranscribe VAD meeting_id=\(meetingID) track=\(trackRole) segments=\(speechSegments.count) audio_segments=\(audioSegments.count) covered=\(String(format: "%.1f", coveredDuration)) duration=\(String(format: "%.1f", duration))"
        )
        ...
        if backend.backend == ONNXGigaAMModelStore.backendIdentifier {
            transcription = try await MeetingRetranscriptionPipeline.transcribeSegmentedAudio(
                ...
                vadSegments: speechSegments,
                trackRole: trackRole,
                diagnosticsLabel: "[guesli-native] source-track retranscribe meeting_id=\(meetingID)",
        ...
        DiagnosticsLog.write(
            "[guesli-native] source-track retranscribe track_done meeting_id=\(meetingID) track=\(trackRole) segments=\(transcription.segments.count) chars=\(MeetingRetranscriptionPipeline.characterCount(transcription.segments)) text_chars=\(segmentedChars) covered=\(String(format: "%.1f", coveredDuration))"
        )
        return MeetingSourceTrackTranscription(
    ...
    private func transcribeFullMeetingSourceTrack(
        ...
        DiagnosticsLog.write("[guesli-native] source-track retranscribe full_track_retry meeting_id=\(meetingID) track=\(trackRole) reason=\(reason)")
        let transcription = try await transcriptionCoordinator.transcribeMeeting(
            at: url,
            samples: wavData.samples,
            backend: backend,
    ...
    private func retranscribeDiarizationSegments(
        ...
        do {
            ...
            DiagnosticsLog.write("[guesli-native] retranscribe diarization failed meeting_id=\(meetingID) error=\(error.localizedDescription)")
            return nil
        }
    }
    ...
    func updateMeetingTitle(id: Int64, title: String) {
        ...
        do {
            ...
            fputs("[guesli-native] failed to update meeting title \(id): \(error)\n", stderr)
        }
        syncAppState()
    }
    ...
    func updateMeetingTranscript(id: Int64, transcript: String) {
        do {
            ...
            fputs("[guesli-native] failed to update meeting transcript \(id): \(error)\n", stderr)
        }
        syncAppState()
    }
    ...
    func updateMeetingManualNotes(id: Int64, notes: String) {
        ...
        do {
            ...
            fputs("[guesli-native] failed to update manual notes for \(id): \(error)\n", stderr)
        }
        syncAppState()
    }
    ...
    private func persistCachedMeetingManualNotes(id: Int64, notes: String, sync: Bool) {
        ...
        do {
            ...
            fputs("[guesli-native] failed to persist manual notes for \(id): \(error)\n", stderr)
        }
        if sync {
            syncAppState()
        }
    ...
    private func flushCachedMeetingTitle(id: Int64) {
        ...
        do {
            ...
            fputs("[guesli-native] failed to flush cached meeting title \(id): \(error)\n", stderr)
        }
    }
    ...
    func createMeetingFromCalendarEvent(_ event: UnifiedCalendarEvent, folderID: Int64?) {
        ...
        if let existing = try? dictationStore.meetingByCalendarOccurrence(occurrence) {
            ...
            syncAppState()
            fputs("[guesli-native] calendar event already exists as meeting \(existing.id), moved to folder\n", stderr)
            return
        }
        ...
        do {
            ...
            scheduleICloudSyncAfterLocalChange()
            syncAppState()
            fputs("[guesli-native] created meeting from calendar event: \(event.title) (folder=\(folderID.map(String.init) ?? "none"))\n", stderr)
        } catch {
            fputs("[guesli-native] failed to create meeting from calendar event: \(error)\n", stderr)
        }
    }
    ...
    func startForegroundMeetingRecording(
        ...
        let didStart = startMeetingRecording(
            ...
            openDocument: presentation.showsNotes,
            endDate: endDate,
    ...
    func startMeetingRecording(
        ...
        do {
            ...
            fputs("[guesli-native] failed to create live meeting: \(error)\n", stderr)
        ...
        meetingStartTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                if self.meetingStartMeetingID == meetingID {
                    fputs("[guesli-native] failed to start meeting: \(error)\n", stderr)
    ...
    func startFollowUpMeeting(fromMeetingID meetingID: Int64) {
        guard !isMeetingRecording(), !isStartingMeetingRecording,
              let clicked = meeting(id: meetingID),
              canStartFollowUpMeeting(clicked) else { return }
    ...
    func meetingThreadContext(for meetingID: Int64) -> MeetingThreadContext? {
        do {
            let ids = try dictationStore.meetingThreadIDs(containing: meetingID)
            guard ids.count > 1, let index = ids.firstIndex(of: meetingID) else { return nil }
            let navigation = try dictationStore.meetingThreadNavigation(containing: meetingID)
            ...
            fputs("[guesli-native] failed to resolve meeting thread for \(meetingID): \(error)\n", stderr)
            return nil
        }
    }
    ...
    func resumeFinishedMeeting(meetingID: Int64) {
        guard !isMeetingRecording(), !isStartingMeetingRecording,
              ...
              canResumeFinishedMeeting(meeting) else { return }
        guard let meetingBackend = normalizeMeetingTranscriptionSelectionForAvailability() else {
            presentErrorAlert(title: "Resume failed", message: "Download a transcription model before recording.")
            return
        }
        ...
        do {
            ...
            fputs("[guesli-native] failed to prepare meeting resume: \(error)\n", stderr)
        ...
        let templateSnapshot = meetingTemplateSnapshot(for: meeting)
        let participants = participantCandidates(forCalendarEventID: meeting.calendarEventID)
        meetingStartTask = Task { @MainActor [weak self] in
            guard let self else { return }
            defer { self.finishMeetingStartAttempt(meetingID: meetingID) }
            do {
                try Task.checkCancellation()
                try await self.startMeetingRecordingWithSystemAudioRecovery(
                    title: meeting.title,
                ...
                if self.meetingStartMeetingID == meetingID {
                    fputs("[guesli-native] failed to resume meeting: \(error)\n", stderr)
                    self.disarmMeetingAutoStop()
                    self.resolveLiveMeetingAfterStartFailure(id: meetingID)
                    self.cancelMeetingRecordingHotkeyToggleAfterFailedStart(meetingID: meetingID)
                    self.meetingMonitor.resumeAfterCooldown()
    ...
    private func startMeetingRecordingWithSystemAudioRecovery(
        ...
        while true {
            ...
            meetingSession.templateSnapshotOverride = templateSnapshot
                ...
                meetingSession.onChunkTranscribed = { [weak self, weak meetingSession] segments, speaker in
                    Task { @MainActor [weak self, weak meetingSession] in
                        ...
                        do {
                            ...
                            fputs("[guesli-native] failed to checkpoint live transcript for meeting \(meetingID): \(error)\n", stderr)
                        }
    ...
    private func beginPendingMeetingJoinRecording(
        ...
        pendingMeetingJoinRecordingTimeoutTask = Task { @MainActor [weak self] in
            ...
            fputs("[guesli-native] Join & Record timed out waiting for meeting activity\n", stderr)
    ...
    private func startPendingMeetingJoinRecordingIfReady(candidate: MeetingCandidate?) {
        ...
        if startForegroundMeetingRecording(
            title: pending.title,
            calendarOccurrence: pending.calendarOccurrence,
    ...
    private func openMeetingURL(_ meetingURL: URL, completion: (@MainActor (Bool) -> Void)? = nil) {
        ...
        guard let appURL = NSWorkspace.shared.urlForApplication(withBundleIdentifier: bundleID) else {
            fputs("[guesli-native] preferred meeting browser not found: \(bundleID); using system default\n", stderr)
            completion?(NSWorkspace.shared.open(meetingURL))
            return
        }
        ...
        NSWorkspace.shared.open([meetingURL], withApplicationAt: appURL, configuration: configuration) { _, error in
            if let error {
                fputs("[guesli-native] preferred meeting browser open failed for \(bundleID): \(error.localizedDescription); using system default\n", stderr)
                Task { @MainActor in
                    completion?(NSWorkspace.shared.open(meetingURL))
                }
            } else {
    ...
    private func restoreResumedMeetingIfNeeded(id: Int64) -> Bool {
        ...
        do {
            guard try dictationStore.restoreResumedMeetingIfNeeded(id: id) else { return false }
        } catch {
            fputs("[guesli-native] failed to restore resumed meeting \(id): \(error)\n", stderr)
            return false
        }
        ...
        if activeMeetingAudioWarning?.meetingID == id { activeMeetingAudioWarning = nil }
        syncAppState()
        historyWindowController?.reload()
        return true
    }
    ...
    private func deleteMeetingDraftAndScheduleSync(id: Int64) {
        do {
            ...
            fputs("[guesli-native] failed to delete meeting draft \(id): \(error)\n", stderr)
        }
    }
    ...
    private func updateMeetingStatusAndScheduleSync(id: Int64, status: MeetingStatus) {
        do {
            ...
            fputs("[guesli-native] failed to update meeting \(id) status to \(status.rawValue): \(error)\n", stderr)
        }
    }
    ...
    func openDiagnosticIncidentIssue(_ incident: DiagnosticIncident) {
        ...
        DispatchQueue.main.async {
            ...
            NSWorkspace.shared.open([url], withApplicationAt: applicationURL, configuration: configuration) { _, error in
                if let error {
                    fputs("[guesli-native] failed to open diagnostic issue URL with activation: \(error)\n", stderr)
                    NSWorkspace.shared.open(url)
                }
            }
        }
    ...
    func stopMeetingRecording() {
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            do {
                ...
                fputs("[guesli-native] meeting transcription failed: \(error)\n", stderr)
                await MainActor.run {
                    _ = self.recordDiagnosticIncident(
                        kind: .meetingProcessingFailed,
    ...
    func persistCompletedMeetingResult(
        ...
        let meetingID: Int64
        let existingMeeting = existingMeetingID.flatMap { meeting(id: $0) }
        let preservesPriorRecording = existingMeetingID.map { pendingResumePriorTranscript[$0] != nil } ?? false
        let savedRecordingPath = preparedRecordingSave.path
            ?? (preservesPriorRecording ? existingMeeting?.savedRecordingPath : nil)
    ...
    private func mergedResumeResult(
        ...
        guard let meetingID,
              ...
              let priorMeeting = meeting(id: meetingID) else { return result }
        let originalStart = ISO8601DateFormatter().date(from: priorMeeting.startTime)
        guard MeetingResumePolicy.hasNewTranscriptContent(prior: prior, new: result.rawTranscript) else {
            return result.overriding(
                startTime: originalStart,
                durationSeconds: priorMeeting.durationSeconds + result.durationSeconds,
                rawTranscript: prior,
                rawOriginalTranscript: priorMeeting.rawOriginalTranscript,
                formattedNotes: priorMeeting.formattedNotes
            )
        }

        let combined = MeetingResumePolicy.combinedResumeTranscript(prior: prior, new: result.rawTranscript)
        let combinedOriginal = MeetingResumePolicy.combinedResumeTranscript(
            prior: priorMeeting.rawOriginalTranscript ?? prior,
            new: result.rawOriginalTranscript ?? result.rawTranscript
        )
        let combinedNotes = [priorMeeting.formattedNotes, result.formattedNotes]
        ...
        return result.overriding(
            ...
            rawOriginalTranscript: combinedOriginal,
            formattedNotes: combinedNotes
        )
    }
        ...
        if let existingMeetingID {
            do {
                ...
                guard try dictationStore.meeting(id: existingMeetingID)?.savedRecordingPath == path else {
                    throw NSError(domain: "GuesliController", code: 1, userInfo: [
    ...
    private func loadPostMeetingSourceSidecar(meeting: MeetingRecord) -> PostMeetingSourceTrackSidecar? {
        ...
        for url in candidatePaths where FileManager.default.fileExists(atPath: url.path) {
            do {
                ...
                DiagnosticsLog.write(
                    "[guesli-native] failed_to_read_post_source_sidecar meeting_id=\(meeting.id) path=\(url.path) error=\(error.localizedDescription)"
                )
            }
        }
        return nil
    ...
    private func promptToSaveMeetingRecording(for title: String) async -> Bool {
        ...
        guard let window = alertPresentationWindow(showHistoryIfNeeded: true) else {
            fputs("[guesli-native] no window available for recording save prompt; saving recording by default\n", stderr)
            return true
        }
    ...
    private func presentAlert(
        ...
        guard let window = alertPresentationWindow(showHistoryIfNeeded: true) else {
            fputs(
                "[guesli-native] unable to present \(fallbackLogContext) alert: \(alert.messageText) - \(alert.informativeText)\n",
                stderr
            )
    ...
    private var isDictationActivityInProgress: Bool {
        dictationState != .idle || dictationStartedAt != nil || isNemotron35Streaming
    }
    ...
    private func ensureDictationBackendReady() -> Bool {
        ...
        guard let message = dictationBackendReadiness.blockingMessage(backendLabel: selectedBackend.label) else {
            return true
        }
        statusBarController?.setStatus(message)
        statusBarController?.refresh()
        switch dictationBackendReadiness {
        case .preparing: indicator.showLoading(message)
        case .failed: indicator.showWarning(message, icon: "!")
        case .ready: break
        }
        return false
    }
    ...
    private func handleDictationAudioSessionEvent(_ event: DictationAudioSessionEvent) {
        ...
        case .stopped(let eventSessionID, let wavURL):
            guard pendingDictationStopSessionID == eventSessionID else {
                fputs("[guesli-native] ignoring stale stopped event\n", stderr)
                if let wavURL {
            ...
            guard dictationAudioSessionManager.currentSessionID == nil else {
                fputs("[guesli-native] ignoring stopped event while a new session is active\n", stderr)
                if let wavURL {
                    try? FileManager.default.removeItem(at: wavURL)
                }
                break
        ...
        case .failed(_, let error):
            fputs("[guesli-native] recorder start failed: \(error)\n", stderr)
            if !isDictationTestMode {
                recordDiagnosticIncident(
    ...
    private func handleStart() {
        ...
        fputs("[guesli-native] recording start\n", stderr)
        meetingMonitor.suppressWhileActive()
    ...
    private func startNemotronStreamingAsync(
        ...
        Task {
            ...
            fputs("[guesli-native] got Nemotron 3.5 transcriber\n", stderr)
            ...
            await MainActor.run {
                guard self.isNemotron35Streaming, self.nemotron35StreamingSessionID == sessionID else {
                    fputs("[guesli-native] Nemotron session cancelled before transcriber ready\n", stderr)
                    return
                }
                ...
                controller.onPartialText = { [weak self] fullText in
                    ...
                    DispatchQueue.main.async {
                        ...
                        fputs("[guesli-native] streaming partial: +\"\(delta)\" (total \(fullText.count) chars)\n", stderr)
                        if !delta.isEmpty {
                ...
                fputs("[guesli-native] Nemotron streaming controller started\n", stderr)
            }
        }
    }
    ...
    private func handleNemotronStreamingStartFailure() {
        fputs("[guesli-native] Nemotron streaming controller failed to start\n", stderr)
        if !isDictationTestMode {
            recordDiagnosticIncident(
    ...
    private func handleNemotronStreamingRuntimeFailure(error: Error, sessionID: UUID) {
        ...
        fputs("[guesli-native] Nemotron streaming failed: \(error)\n", stderr)
        if !isDictationTestMode {
            recordDiagnosticIncident(
    ...
    private func handleToggleStart(outputMode: DictationOutputMode? = nil) -> Bool {
        ...
        fputs("[guesli-native] toggle dictation start\n", stderr)
        if dictationLatencyTraceID == nil {
        ...
        if isStreamingDictationBackend {
            if #available(macOS 15, *) {
                ...
                meetingMonitor.refreshState()
                fputs("[guesli-native] Nemotron streaming toggle mode active\n", stderr)
    ...
    private func handleToggleStop() {
        fputs("[guesli-native] toggle dictation stop\n", stderr)
    ...
    private func handleStop() {
        ...
        if isNemotron35Streaming {
            ...
            if #available(macOS 15, *), let controller = _streamingDictationController as? StreamingDictationController {
                ...
                fputs("[guesli-native] Nemotron streaming stop, controller not ready (short press)\n", stderr)
    ...
    private func cancelDictationAudioSessionForMeetingRecordingIfNeeded() {
        ...
        fputs("[guesli-native] cancelling dictation audio session because meeting is active\n", stderr)
    ...
    private func finishNemotronStreamingStop(
        ...
        guard isNemotron35Streaming, nemotron35StreamingSessionID == sessionID else {
            fputs("[guesli-native] ignoring stale Nemotron stop completion\n", stderr)
            return
        }
        isNemotron35Streaming = false
        _streamingDictationController = nil
        ...
        fputs("[guesli-native] Nemotron streaming stop, got \(finalText.count) chars\n", stderr)
        ...
        setState(.idle)
        meetingMonitor.resumeAfterCooldown()
        fputs("[guesli-native] Nemotron streaming done (\(String(format: "%.1f", duration))s)\n", stderr)
    ...
    private func finishStandardDictationStop(wavURL stoppedWavURL: URL?, startedAt: Date) {
        ...
        guard let wavURL = stoppedWavURL else {
            fputs("[guesli-native] stop without wav\n", stderr)
            clearCapturedDictationSessionContext()
            resetDictationOutputMode()
            setState(.idle)
            meetingMonitor.resumeAfterCooldown()
        ...
        let duration = max(Date().timeIntervalSince(startedAt), 0)
        if duration < 0.3 {
            fputs("[guesli-native] discarded short recording\n", stderr)
            try? FileManager.default.removeItem(at: wavURL)
            if isDictationTestMode {
        ...
        let task = Task { [weak self] in
            ...
            do {
                ...
                fputs("[guesli-native] test dictation cancelled\n", stderr)
                await MainActor.run {
                    self.clearCapturedDictationSessionContext()
                    self.resetDictationOutputMode()
                    self.setState(.idle)
                ...
                fputs("[guesli-native] transcription failed: \(error)\n", stderr)
                await MainActor.run {
    ...
    private func userFacingDictationTestError(_ error: Error) -> String {
        ...
        if nsError.domain == "GuesliTranscriptionRuntime" {
            switch nsError.code {
            case 1:
