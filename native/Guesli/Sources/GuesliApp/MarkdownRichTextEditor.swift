...
private extension NSAttributedString.Key {
    static let guesliMarkdownStrong = NSAttributedString.Key("com.guesli.markdown.strong")
    static let guesliMarkdownEmphasis = NSAttributedString.Key("com.guesli.markdown.emphasis")
    static let guesliMarkdownCode = NSAttributedString.Key("com.guesli.markdown.code")
}
    ...
    final class Coordinator: NSObject, NSTextViewDelegate {
        ...
        private func inlineAttributedString(
            ...
            for run in parsed.runs {
                ...
                if isCode {
                    ...
                    runAttributes[.guesliMarkdownCode] = true
                }
                if isEmphasized {
                    font = italicVersion(of: font)
                    runAttributes[.guesliMarkdownEmphasis] = true
                }
                if isStrong {
                    font = boldVersion(of: font)
                    runAttributes[.guesliMarkdownStrong] = true
                }
        ...
        private func markdownInline(from storage: NSTextStorage, contentRange: NSRange) -> String {
            ...
            storage.enumerateAttributes(in: contentRange) { attrs, range, _ in
                ...
                let isStrong = attrs[.guesliMarkdownStrong] as? Bool == true
                    || (!isHeading && isBold(font))
                let isEmphasized = attrs[.guesliMarkdownEmphasis] as? Bool == true
                let isCode = attrs[.guesliMarkdownCode] as? Bool == true
