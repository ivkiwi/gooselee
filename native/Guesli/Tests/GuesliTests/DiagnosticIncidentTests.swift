...
@Suite("DiagnosticIncident", .guesliHermeticSupport)
struct DiagnosticIncidentTests {
    ...
    func issueBodyAvoidsPrivatePayloads() {
        ...
        let error = NSError(
            domain: "GuesliTranscriptionRuntime",
            code: 7,
            userInfo: [
    ...
    func domainFallbackCoversSwiftEnumErrors() {
        let meaning = DiagnosticErrorCatalog.meaning(
            domain: "GuesliApp.MeetingLifecycleError",
            code: "0"
        )
    ...
    func controllerWiresHardFailureIncidentHooks() throws {
        let source = try sourceFile(named: "GuesliController.swift")
        let expectedStages = [
            "create_live_meeting",
    ...
    func diagnosticIncidentPortHasNoTelemetryDeckDependency() throws {
        let sourceFiles = [
            ...
            try sourceFile(named: "GuesliController.swift"),
...
@Suite("DiagnosticIncidentReporter", .guesliHermeticSupport)
@MainActor
