...
struct ModelDownloadCoordinatorTests {
    ...
    @Test("supported managed ASR plans use their official Hugging Face repositories")
    func supportedManagedASRPlansUseOfficialRepositories() throws {
        let root = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }

        let parakeetV3 = ManagedASRModelPlans.parakeetV3(modelsRoot: root)
        #expect(parakeetV3.repository == "FluidInference/parakeet-tdt-0.6b-v3-coreml")
        ...
        #expect(realtimeEOU.repository == "FluidInference/parakeet-realtime-eou-120m-coreml")
    ...
    func etagMismatchRestartsPartial() async throws {
        ...
        try state.write(to: directory.appendingPathComponent(".guesli-download-state.json"))
        let manifest = ModelDownloadManifest(
            id: "etag",
            version: "1",
            files: [ModelDownloadFile(relativePath: "model.bin", remoteURL: try #require(URL(string: "https://example.com/model")), expectedByteCount: 5)],
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-download-tests", isDirectory: true)
