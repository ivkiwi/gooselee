...
struct AppleShortcutsIntegrationTests {
    ...
    func declaresControlIntents() throws {
        let source = try String(
            contentsOf: repositoryRoot.appendingPathComponent(
                "native/Guesli/Sources/GuesliAppShell/GuesliAppIntents.swift"
            ),
            encoding: .utf8
        )
    ...
    func controllerEntryPoints() throws {
        let source = try String(
            contentsOf: repositoryRoot.appendingPathComponent(
                "native/Guesli/Sources/GuesliApp/GuesliController.swift"
            ),
            encoding: .utf8
        )

        #expect(source.contains("public static weak var current: GuesliController?"))
