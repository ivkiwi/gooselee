...
const BACKUP_STORAGE_KEY = "guesliMeetSpeakerBridge.timeline.v1";
...
const speakerDetection = globalThis.GuesliMeetSpeakerDetection;
...
async function postBridgePayload(payload) {
  const response = await sendBackgroundMessage({ type: "guesli.postBridgePayload", payload });
  if (!response?.ok) {
