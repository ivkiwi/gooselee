...
struct TranscribeCommand: AsyncParsableCommand {
    ...
    var summarize = false
    @Flag(name: .long, help: "Save the transcript as an imported Guesli meeting.")
    ...
    mutating func validate() throws {
        ...
        guard GuesliAudioFilePreparer.isSupportedFileURL(url) else {
...
extension TranscribeJSONPayload {
    init(_ result: GuesliAudioTranscriptionResult) {
        self.init(
...
struct GuesliAudioTranscriptionRequest {
    let sourceURL: URL
...
struct GuesliAudioTranscriptionPipeline {
    ...
    init(
        audioPreparer: AudioPreparing = GuesliAudioFilePreparer(),
        transcriber: AudioTranscribing = CoordinatorAudioTranscriber(),
        summarizer: MeetingSummarizing = ConfiguredCLIMeetingSummarizer(),
        dataChangePoster: @escaping () -> Void = GuesliNotifications.postDataDidChange
    ) {
    ...
    func run(request: GuesliAudioTranscriptionRequest, context: CLIContext) async throws -> GuesliAudioTranscriptionResult {
        ...
        fputs("[guesli-cli] preparing audio...\n", stderr)
        ...
        fputs("[guesli-cli] loading \(request.model.rawValue) and transcribing...\n", stderr)
        ...
        return GuesliAudioTranscriptionResult(
            title: title,
...
struct GuesliAudioFilePreparer: AudioPreparing {
    ...
    private func temporaryWAVURL() throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-cli-import", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    private func decodeAssetReaderToTemporaryWAV(sourceURL: URL) async throws -> (wavURL: URL, sampleCount: Int) {
        ...
        let wavURL = try CLIWavWriter.temporaryWAVURL(directoryName: "guesli-cli-import")
        do {
