...
struct MeetingListItemView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: GuesliTheme.spacing8) {
            ...
            HStack(spacing: GuesliTheme.spacing4) {
                ...
                if let name = currentFolderName {
                    ...
                    .foregroundStyle(GuesliTheme.accent.opacity(0.8))
                }
            }
        ...
        .background(isSelected ? GuesliTheme.surfaceSelected : GuesliTheme.backgroundRaised)
        .clipShape(RoundedRectangle(cornerRadius: GuesliTheme.cornerLarge))
        .overlay(
            RoundedRectangle(cornerRadius: GuesliTheme.cornerLarge)
                .strokeBorder(
                    isSelected ? GuesliTheme.accent.opacity(0.35) : GuesliTheme.surfaceBorder,
                    lineWidth: 1
                )
        )
        ...
        if record.followUpToID != nil || hasFollowUps {
            HStack(spacing: 4) {
                if record.followUpToID != nil {
                    relationshipIcon("arrow.turn.down.right", help: "Follow-up meeting")
                }
                if hasFollowUps {
                    relationshipIcon("arrow.triangle.branch", help: "Has follow-up meetings")
                }
            }
        }
    }
    ...
    private func relationshipIcon(_ systemName: String, help: String) -> some View {
        Image(systemName: systemName)
            ...
            .foregroundStyle(GuesliTheme.accent.opacity(0.9))
        ...
        Button {
            ...
            Image(systemName: record.folderID != nil ? "folder.fill" : "folder.badge.plus")
                ...
                .foregroundStyle(
                    record.folderID != nil
                        ? GuesliTheme.accent
                        : (isHovering ? GuesliTheme.textSecondary : GuesliTheme.textTertiary)
                )
                .frame(width: 24, height: 24)
                .contentShape(Rectangle())
        }
    ...
    private var deleteButton: some View {
        Button {
            ...
            Image(systemName: "trash")
                ...
                .foregroundStyle(
                    isHovering
                        ? GuesliTheme.recording.opacity(0.85)
                        : GuesliTheme.textTertiary
                )
                .frame(width: 24, height: 24)
                .contentShape(Rectangle())
        }
    ...
    private func sourceBadge(icon: String, label: String, help: String) -> some View {
        ...
        .foregroundStyle(isImportedAudio ? GuesliTheme.accent : GuesliTheme.textSecondary)
        .padding(.horizontal, 6)
        .padding(.vertical, 2)
        .background((isImportedAudio ? GuesliTheme.accent : GuesliTheme.textSecondary).opacity(0.12))
        .clipShape(Capsule())
        .help(help)
        .accessibilityLabel(help)
    }
