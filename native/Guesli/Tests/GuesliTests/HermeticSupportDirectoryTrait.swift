...
struct HermeticSupportDirectoryTrait: SuiteTrait, TestTrait, TestScoping {
    ...
    func provideScope(
        ...
        GuesliPaths.markRunningTestsForCurrentProcess()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("guesli-test-support", isDirectory: true)
            .appendingPathComponent(safeDirectoryName(for: test), isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)

        try await GuesliPaths.$testSupportDirectoryRoot.withValue(root) {
            try await function()
        }
    }
...
extension Trait where Self == HermeticSupportDirectoryTrait {
    static var guesliHermeticSupport: Self { Self() }
}
