...
enum ONNXGigaAMModelStore {
    ...
    static func cacheDirectory(fileManager: FileManager = .default) -> URL {
        GuesliPaths.defaultSupportDirectoryURL(
    ...
    static func downloadIfNeeded(progress: ((Double, String?) -> Void)? = nil) async throws -> URL {
        ...
        GuesliPaths.preconditionSafeForTestWrite(root)
        ...
        GuesliPaths.preconditionSafeForTestWrite(finalDirectory)
...
final class ONNXGigaAMHelperProcess: @unchecked Sendable {
    ...
    private static func helperURL() throws -> URL {
        if let override = ProcessInfo.processInfo.environment["GUESLI_ONNX_GIGAAM_HELPER"], !override.isEmpty {
            let url = URL(fileURLWithPath: override)
            guard FileManager.default.isExecutableFile(atPath: url.path) else {
                throw helperError("GUESLI_ONNX_GIGAAM_HELPER is not executable: \(url.path)")
            }
            return url
        }
