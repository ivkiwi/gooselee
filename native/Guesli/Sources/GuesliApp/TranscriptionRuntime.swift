...
actor TranscriptionCoordinator {
    ...
    private func logPostProcPair(raw: String, processed: String, asr: String, model: String? = nil) {
        ...
        GuesliPaths.preconditionSafeForTestWrite(logURL)
    ...
    func preload(
        ...
        do {
            ...
            fputs("[guesli-native] preload failed for \(backend.backend)/\(backend.model): \(error)\n", stderr)
        }
    }
    ...
    func preloadRequired(
        ...
        case "nemotron35":
            if #available(macOS 15, *) {
                ...
                fputs("[guesli-native] Nemotron 3.5 warmup: running silent chunk for ANE compilation...\n", stderr)
                ...
                fputs("[guesli-native] Nemotron 3.5 warmup complete\n", stderr)
            } else {
                throw NSError(domain: "GuesliTranscriptionRuntime", code: 1, userInfo: [
        ...
        default:
            throw NSError(domain: "GuesliTranscriptionRuntime", code: 5, userInfo: [
    ...
    private func finishDiarizerLoad(
        ...
        if timedOut {
            outcome = .timedOut
            fputs("[guesli-native] Diarization load timed out (non-critical)\n", stderr)
        } else {
            switch result {
            case .success(let models):
                ...
                outcome = .succeeded
                fputs("[guesli-native] Speaker diarization loaded (compute: \(policy.computePolicy.rawValue))\n", stderr)
            case .failure(let error):
                outcome = error is CancellationError ? .cancelled : .failed
                fputs("[guesli-native] Diarization load failed (non-critical): \(error)\n", stderr)
            }
        }
        DiagnosticsLog.write(
    ...
    func transcribeMeetingChunk(
        ...
        if let vadManager {
            do {
                ...
                if !hasSpeech {
                    fputs("[guesli-native] VAD: chunk is silent, skipping transcription\n", stderr)
                    return SpeechTranscriptionResult(text: "", segments: [])
                }
            } catch {
                fputs("[guesli-native] VAD check failed, transcribing anyway: \(error)\n", stderr)
            }
        }
    ...
    func diarizeSystemAudio(samples: [Float]) throws -> DiarizationResult? {
        guard let diarizerManager, diarizerManager.isAvailable else {
            fputs("[guesli-native] diarization not available, skipping\n", stderr)
            return nil
        }
        fputs("[guesli-native] running speaker diarization on system audio...\n", stderr)
        ...
        fputs("[guesli-native] diarization complete: \(result.segments.count) segments, \(speakerCount) speakers\n", stderr)
        return result
    }
    ...
    private func transcribeWithFluidAudio(
        ...
        fputs("[guesli-native] transcribing with FluidAudio: \(url.lastPathComponent)\n", stderr)
        let result = try await fluidTranscriber.transcribe(wavURL: url, language: language.isoCode)
        fputs("[guesli-native] FluidAudio result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
    ...
    private func transcribeWithONNXGigaAM(url: URL, samples: [Float]? = nil) async throws -> SpeechTranscriptionResult {
        fputs("[guesli-native] transcribing with ONNX GigaAM E2E CTC INT8: \(url.lastPathComponent)\n", stderr)
        ...
        fputs("[guesli-native] ONNX GigaAM result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
        let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
        return SpeechTranscriptionResult(
            text: text,
    ...
    private func transcribeWithONNXGigaAMBatch(samplesBatch: [[Float]]) async throws -> [SpeechTranscriptionResult] {
        fputs("[guesli-native] transcribing \(samplesBatch.count) segments with ONNX GigaAM batch\n", stderr)
        let results = try await onnxGigaAMTranscriber.transcribe(
            samplesBatch: samplesBatch,
            sampleRate: 16_000
        )
        fputs("[guesli-native] ONNX GigaAM batch completed (items=\(results.count))\n", stderr)
    ...
    private func transcribeWithNemotron35(url: URL, samples: [Float]? = nil) async throws -> SpeechTranscriptionResult {
        if #available(macOS 15, *) {
            fputs("[guesli-native] transcribing with Nemotron 3.5: \(url.lastPathComponent)\n", stderr)
            ...
            fputs("[guesli-native] Nemotron 3.5 result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
            return SpeechTranscriptionResult(
                text: text,
            ...
            throw NSError(domain: "Guesli", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "Nemotron 3.5 requires macOS 15 or later.",
            ])
        }
    }
